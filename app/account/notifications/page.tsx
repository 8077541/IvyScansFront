  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        {unreadCount > 0 && (
          <Button onClick={markAllAsRead} className="bg-green-400 hover:bg-green-500">
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {notifications.map((notification) => (
            <Card
              key={notification.id}
              className={`transition-colors ${!notification.read ? "border-green-500/50 bg-green-500/5" : ""}`}
                ...
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <h3 className={`font-medium ${!notification.read ? "text-green-500" : ""}`}>
                        {notification.type === "NEW_CHAPTER" ? "New Chapter Available" : "System Message"}
                      </h3>
                      {!notification.read && <Badge className="bg-green-500 text-white">New</Badge>}
                    </div>
                  ...
                  <div>
                    {notification.type === "NEW_CHAPTER" && notification.data && (
                      <Button asChild size="sm" className="bg-green-400 hover:bg-green-500">
                        <Link href={`/comics/${notification.data.comicId}/chapters/${notification.data.chapterNumber}`}>
                          Read Now
                        </Link>
                      </Button>
                    )}
