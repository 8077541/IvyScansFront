...
import { ChevronLeft, ChevronRight, Settings, ArrowUp, Maximize, Minimize } from "lucide-react"
...
export default function ChapterPage({ params }: ChapterPageProps) {
  ...
  useEffect(() => {
    const recordReadingHistory = async () => {
      ...
      if (historyRecorded || !isAuthenticated || !isValidParams || !comicId || !comicTitle) return
      ...
      try {
        ...
        console.log("Reading history recorded:", comicId, actualChapterId, chapterNumber)
    ...
    const fetchChapterData = async () => {
      ...
      try {
        ...
        if (chapters) {
          ...
          setPrevChapter(currentIndex > 0 ? chapterNumbers[currentIndex - 1] : null)
          setNextChapter(currentIndex < chapterNumbers.length - 1 ? chapterNumbers[currentIndex + 1] : null)
        }
    ...
    return (
      <main className="reading-container">
        <div className="container mx-auto px-4 py-20 flex flex-col items-center justify-center min-h-[50vh]">
          <h1 className="text-2xl font-bold text-red-500 mb-4">Error: Invalid Parameters</h1>
          <p className="text-muted-foreground mb-6">The comic ID or chapter number is missing or invalid.</p>
          <Button asChild>
    ...
    <main className="reading-container">
          ...
          <div className="flex items-center gap-4">
            ...
            {chapterData && (
              <div className="hidden md:flex items-center gap-2 ml-4 flex-1 max-w-md">
                ...
                <Progress value={(chapterNumber / (chapterData.totalChapters || 10)) * 100} className="h-2" />
          ...
          <div className="flex items-center gap-2">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem
                  onClick={() => setReadingDirection(readingDirection === "vertical" ? "horizontal" : "vertical")}
                >
                  Reading Direction:{" "}
                ...
                <DropdownMenuItem onClick={() => setImageQuality(imageQuality === "high" ? "medium" : "high")}>
      ...
      <div className="container mx-auto px-0 md:px-4 pt-20 pb-20 comic-reader">
        {isLoading ? (
          ...
          <div
            className={`flex ${readingDirection === "vertical" ? "flex-col" : "flex-row overflow-x-auto"} items-center gap-1`}
          >
            {chapterData?.images.map((image, index) => (
              <div key={index} className={`${readingDirection === "vertical" ? "w-full max-w-3xl" : "min-w-fit"}`}>
                <Image
