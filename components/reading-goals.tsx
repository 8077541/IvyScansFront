...
export function ReadingGoals() {
  ...
  const [editingGoal, setEditingGoal] = useState<"daily" | "weekly" | "monthly">("daily")
  ...
  return (
    <>
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <div className="flex items-center">
                  <Trophy
                    className={`h-4 w-4 mr-2 ${goals.daily.current >= goals.daily.target ? "text-yellow-400" : "text-muted-foreground"}`}
                  />
                  <h3 className="text-sm font-medium">Daily Goal</h3>
                </div>
                <div className="text-sm">
                  <span className={goals.daily.current >= goals.daily.target ? "text-green-400 font-medium" : ""}>
                    {goals.daily.current}
                  </span>{" "}
              ...
              <Progress
                ...
                className="h-2"
                indicatorClassName={goals.daily.current >= goals.daily.target ? "bg-green-400" : ""}
              />
              <p className="text-xs text-muted-foreground">
                {goals.daily.current >= goals.daily.target
                  ? "Goal completed! Great job!"
                  : `${goals.daily.target - goals.daily.current} more chapters to reach your daily goal`}
              </p>
            </div>
          )}
          ...
          {goals.weekly.enabled && (
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <div className="flex items-center">
                  <TrendingUp
                    className={`h-4 w-4 mr-2 ${goals.weekly.current >= goals.weekly.target ? "text-yellow-400" : "text-muted-foreground"}`}
                  />
                  <h3 className="text-sm font-medium">Weekly Goal</h3>
                </div>
                <div className="text-sm">
                  <span className={goals.weekly.current >= goals.weekly.target ? "text-green-400 font-medium" : ""}>
              ...
              <Progress
                ...
                className="h-2"
                indicatorClassName={goals.weekly.current >= goals.weekly.target ? "bg-green-400" : ""}
              />
              <p className="text-xs text-muted-foreground">
                {goals.weekly.current >= goals.weekly.target
                  ? "Weekly goal achieved!"
                  : `${goals.weekly.target - goals.weekly.current} more chapters this week`}
              </p>
            </div>
          )}
          ...
          {goals.monthly.enabled && (
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <div className="flex items-center">
                  <Calendar
                    className={`h-4 w-4 mr-2 ${goals.monthly.current >= goals.monthly.target ? "text-yellow-400" : "text-muted-foreground"}`}
                  />
                  <h3 className="text-sm font-medium">Monthly Goal</h3>
                </div>
                <div className="text-sm">
                  <span className={goals.monthly.current >= goals.monthly.target ? "text-green-400 font-medium" : ""}>
              ...
              <Progress
                ...
                className="h-2"
                indicatorClassName={goals.monthly.current >= goals.monthly.target ? "bg-green-400" : ""}
              />
              <p className="text-xs text-muted-foreground">
                {goals.monthly.current >= goals.monthly.target
                  ? "Monthly goal achieved!"
                  : `${goals.monthly.target - goals.monthly.current} more chapters this month`}
              </p>
            </div>
          )}
        </CardContent>
        <CardFooter className="flex justify-between border-t pt-4">
          <Button variant="outline" size="sm" onClick={() => handleEditGoal("daily")}>
            Edit Daily Goal
          </Button>
          <Button variant="outline" size="sm" onClick={() => handleEditGoal("weekly")}>
            Edit Weekly Goal
          </Button>
          <Button variant="outline" size="sm" onClick={() => handleEditGoal("monthly")}>
            Edit Monthly Goal
          </Button>
        </CardFooter>
      </Card>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Edit {editingGoal.charAt(0).toUpperCase() + editingGoal.slice(1)} Reading Goal</DialogTitle>
            <DialogDescription>Set your target number of chapters to read {editingGoal}.</DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                min="1"
                value={newTarget}
                onChange={(e) => setNewTarget(Number.parseInt(e.target.value) || 1)}
              />
            </div>
