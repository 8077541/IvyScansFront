...
export const comicService = {
  ...
  async getAllComics(params?: {
    ...
    try {
      ...
      const url = `${API_BASE_URL}${COMIC_ENDPOINTS.ALL}?${queryParams.toString()}`
      logApiCall("GET", url)
  ...
  async getFeaturedComics(): Promise<Comic[]> {
    if (USE_MOCK_API) {
      ...
      return mockComics.filter((comic) => comic.featured === true)
    }
  ...
  async getComicById(id: string): Promise<Comic> {
    ...
    try {
      ...
      if (cachedData) {
        try {
          ...
          if (cacheTime && Date.now() - cacheTime < 5 * 60 * 1000) {
            console.log(`[API] Using cached data for comic ${id}`)
      ...
      const data = await handleResponse<Comic>(response)
  ...
  ): Promise<{ images: string[]; title?: string; number: number; totalChapters?: number; id?: string }> {
    // Validate parameters
    try {
      validateParam(id, "comicId")
  ...
  async searchComics(query: string): Promise<Comic[]> {
    ...
    try {
      const url = `${API_BASE_URL}${COMIC_ENDPOINTS.SEARCH}?q=${encodeURIComponent(query)}`
      logApiCall("GET", url)
...
export const userService = {
  ...
  async addToHistory(comicId: string, chapterId: string, chapterNumber: number): Promise<ReadingHistoryItem> {
    if (USE_MOCK_API) {
      await delay(300)
      logApiCall("POST", "MOCK /user/history", { comicId, chapterId, chapterNumber })
