...
export function checkParams(params: Record<string, any> | Promise<Record<string, any>>, functionName: string) {
  if (process.env.NODE_ENV === "development") {
    const checkParamsAsync = async () => {
      ...
      if (invalidParams.length > 0) {
        console.warn(`⚠️ ${functionName} called with invalid parameters:`, invalidParams)
    ...
    checkParamsAsync().catch((err) => console.error("Error checking params:", err))
